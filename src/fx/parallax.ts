/**
 * Pointer drift and scroll drift — small, slow, optional.
 *
...
interface Drift { el: HTMLElement; amount: number }
...
export function bindParallax(root: ParentNode = document): void {
  ...
  drifts = $$<HTMLElement>('[data-drift]', scope).map((el) => ({ el, amount: Number(el.dataset.drift) || 0.04 }));
...
addTick((_dt, _t, scrollY) => {
  ...
  for (const m of mouses) {
    m.el.style.transform = `translate3d(${(px * m.amount * 100).toFixed(2)}px, ${(py * m.amount * 100).toFixed(2)}px, 0)`;
  }
});
