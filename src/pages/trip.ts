...
export async function tripPage(ctx: Ctx): Promise<View> {
  ...
  <div class="sheet">
    <header class="story__head rv">
      <p class="mono">${esc(trip.place)}</p>
      <h1 class="story__title">${esc(trip.title)}</h1>
      <p class="story__meta">${dateRange(trip.start, trip.end)} · ${trip.days} 天</p>
      <p class="story__lead">${esc(trip.summary)}</p>
    </header>
    ${
      trip.cover
        ? `<figure class="story__cover rv"><img src="${esc(trip.cover)}" alt="${esc(trip.title)}" decoding="async" /></figure>`
        : ''
    }
    ...
    <footer class="story__foot">
      ...
      <a class="backlink" href="/travel" data-link>← 回到旅途</a>
    </footer>
  </div>
</article>`,
